...
/**
 * Minimal interface for the authenticated user on the request.
 * The full DTO is product-specific; this guard only needs the id.
 */
...
/**
 * Minimal interface for the RBAC service.
 * Products inject a concrete implementation via RBAC_SERVICE token.
 */
export interface RbacServiceInterface {
  isSuperAdmin(userId: string): Promise<boolean>;
  isAdmin(userId: string, orgSlug: string): Promise<boolean>;
  hasPermission(
    userId: string,
    orgSlug: string,
    permission: string,
...
/** Injection token — products provide the concrete RbacService */
export const RBAC_SERVICE = Symbol('RBAC_SERVICE');

/**
 * Guard to enforce permission-based access control (In-Process)
 *
 ...
 */
@Injectable()
export class InProcessRbacGuard implements CanActivate {
  private readonly logger = new Logger(InProcessRbacGuard.name);

  constructor(
    private readonly reflector: Reflector,
    @Inject(RBAC_SERVICE)
    private readonly rbacService: RbacServiceInterface,
  ) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!user || !user.id) {
      this.logger.warn(
        '[InProcessRbacGuard] No user found on request - JwtAuthGuard should run first',
      );
    ...
    try {
      ...
      this.logger.warn(
        `[InProcessRbacGuard] Super admin check failed, continuing with normal permission check: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    if (permission.startsWith('admin:')) {
      try {
        ...
        // Log error but continue with normal permission check
        this.logger.warn(
          `[InProcessRbacGuard] Admin check failed, continuing with normal permission check: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
