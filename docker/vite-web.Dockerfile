...
# Login persona picker (Demo | Admin). Root .env is not copied into the image,
# so these must arrive as build args or the picker has nothing to fill.
ARG VITE_DEMO_USER_EMAIL=
ARG VITE_DEMO_USER_PASSWORD=
ARG VITE_ADMIN_USER_EMAIL=
ARG VITE_ADMIN_USER_PASSWORD=
...
ENV VITE_SUPABASE_URL=$VITE_SUPABASE_URL \
    ...
    VITE_ADMIN_USER_EMAIL=$VITE_ADMIN_USER_EMAIL \
    VITE_ADMIN_USER_PASSWORD=$VITE_ADMIN_USER_PASSWORD
