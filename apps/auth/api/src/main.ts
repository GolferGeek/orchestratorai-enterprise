...
type ProviderSelector = 'AUTH_PROVIDER' | 'CONFIG_PROVIDER' | 'DB_PROVIDER';
...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('OrchestratorAI Auth API')
    .setDescription(
      'Standalone authentication and authorization service. Issues/validates JWT tokens, manages orgs/users/roles/entitlements.',
    )
    .setVersion('1.0')
    .addBearerAuth(
      {
    ...
    .addTag('auth', 'Login, logout, token refresh, JWT validation')
    .addTag('organizations', 'Organization CRUD and settings')
    .addTag('users', 'User management within orgs')
    .addTag('roles', 'RBAC role definitions')
    .addTag('permissions', 'Permission checks')
    .addTag('entitlements', 'Which products each org can access')
  ...
  // Auth API port: AUTH_API_PORT (preferred) or API_PORT (fallback)
  const portEnvVar = process.env.AUTH_API_PORT ? 'AUTH_API_PORT' : 'API_PORT';
  const portValue = process.env.AUTH_API_PORT || process.env.API_PORT;

  if (!portValue) {
    throw new Error(
      'AUTH_API_PORT environment variable is required. ' +
        'Set AUTH_API_PORT=6100 in your .env file.',
    );
  }
  const port = parseInt(portValue, 10);

  startupLogger.log(`[STARTUP] Using port from ${portEnvVar}=${port}`);
  startupLogger.log(`[STARTUP] Starting Auth API on port ${port}...`);
