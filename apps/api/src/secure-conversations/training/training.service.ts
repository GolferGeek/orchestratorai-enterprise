...
/**
 * TrainingService — Built-in training docs and guided scenarios for Secure Conversations.
 *
 * Secure Conversations includes embedded training content to help users understand:
 ...
 *
 * This content is served by the API and displayed in the Secure Conversations web UI.
 */
...
export class TrainingService {
  listScenarios(): ScenarioDescriptor[] {
    return [
        ...
        steps: [
          {
            ...
            description: 'Send the external agent base URL to Secure Conversations',
            action: 'POST /registry/agents/discover { "url": "https://external-agent.example.com" }',
            expectedOutcome: 'Secure Conversations fetches .well-known/agent.json and returns agent info',
          ...
          },
          {
            ...
            description: "Secure Conversations automatically adds the agent's origin to the trusted list",
            action: 'Review Secure Conversations security logs',
      ...
      },
      {
        ...
        description: 'An external agent sends a JSON-RPC 2.0 task request to Secure Conversations',
        flow: 'inbound',
        ...
        steps: [
          {
            ...
            expectedOutcome: 'Secure Conversations validates origin, rate limit, format, and signature',
            securityLayer: 'identity',
          },
          {
            step: 2,
            title: 'Secure Conversations validates the request',
            description: 'Origin validation, rate limiting, JSON-RPC format check, signature verification',
            action: 'Secure Conversations processes security stack',
            expectedOutcome: 'All checks pass — request proceeds to routing',
            securityLayer: 'trust',
          },
          {
            step: 3,
            title: 'Secure Conversations routes to internal agent',
            description: 'Method name determines which internal agent handles the request',
            action: 'agents.* → unified Agents module, workflows.* → unified Workflows module',
          ...
          },
          {
            ...
            action: 'Secure Conversations returns { jsonrpc: "2.0", id, result: {...} }',
      ...
      },
      {
        ...
        description: 'Secure Conversations sends a signed A2A request to a registered external agent',
        flow: 'outbound',
        difficulty: 'intermediate',
        estimatedMinutes: 10,
        steps: [
          {
            ...
            description: 'Internal system or admin triggers an outbound request via Secure Conversations',
            action: 'POST /a2a/send { "targetAgentId": "...", "method": "...", "params": {...} }',
            expectedOutcome: 'Secure Conversations looks up agent in registry',
          },
          {
            step: 2,
            title: 'Secure Conversations signs the request',
      ...
      },
      {
        ...
        description: 'Observe Secure Conversations reject an inbound request due to security violations',
        flow: 'security',
        difficulty: 'advanced',
        estimatedMinutes: 15,
        steps: [
  ...
  getDocumentation() {
    return {
      overview: {
        title: 'Secure Conversations — External A2A Communication Gateway',
        description: `
Secure Conversations is the OrchestratorAI Enterprise module responsible for external agent-to-agent
communication. It sits at the trust boundary between internal agents and workflows and
...
- Timestamp validation (5-minute acceptance window)
      ...
      runtime: {
        api: 'Unified platform API',
        web: 'Unified platform web',
      },
      keyEndpoints: [
        {
          method: 'GET',
          path: '/secure-conversations/.well-known/agent.json',
          description: 'Agent card discovery — external agents use this to discover Secure Conversations capabilities',
        },
        {
          method: 'POST',
          path: '/secure-conversations/a2a/tasks',
          description: 'Inbound A2A — external agents send tasks here (JSON-RPC 2.0)',
        },
        {
          method: 'POST',
          path: '/secure-conversations/a2a/send',
          description: 'Outbound A2A — send tasks to registered external agents',
        },
        {
          method: 'GET',
          path: '/secure-conversations/registry/agents',
          description: 'List registered external agents',
        },
        {
          method: 'POST',
          path: '/secure-conversations/registry/agents/discover',
          description: 'Discover and register an external agent via .well-known/agent.json',
        },
        {
          method: 'GET',
          path: '/secure-conversations/stream/events',
          description: 'SSE stream — real-time Secure Conversations events (inbound, outbound, security)',
        },
      ],
      securityLayers: [
        {
