 ...
 * External agents discover Secure Conversations capabilities by fetching this endpoint.
 * Format follows JSON-RPC 2.0 A2A agent card standard from transport-types.
 */
// A2A agent discovery
@Public()
@Controller('secure-conversations/.well-known')
export class WellKnownController {
  @Get('agent.json')
  getAgentCard() {
    const baseUrl =
      process.env.SECURE_CONVERSATIONS_BASE_URL ??
      process.env.PLATFORM_API_URL ??
      `http://localhost:${process.env.PORT ?? '6700'}/secure-conversations`;

    return {
      id: 'orchestratorai-secure-conversations',
      name: 'OrchestratorAI Secure Conversations',
