...
export class WorkflowRagService {
  ...
  async getContext(params: RagContextParams): Promise<string> {
    ...
    if (!collection) {
      throw new Error(
        `RAG collection '${params.collectionSlug}' was not found for org '${params.orgSlug}'`,
      );
    }
  ...
  async smartContext(
    ...
    if (active.length === 0) {
      throw new Error(
        `No active RAG collections with documents found for org '${params.orgSlug}'`,
      );
    }
    ...
    if (isSingleStream || active.length <= (params.maxCollections ?? 3)) {
      ...
      if (!Array.isArray(selectedSlugs)) {
        throw new Error(
          `RAG collection router returned non-array JSON: ${routingResponse.text}`,
        );
      }
      validSlugs = selectedSlugs.filter(
        (slug): slug is string =>
          typeof slug === 'string' && active.some((c) => c.slug === slug),
      );
      if (validSlugs.length === 0) {
        throw new Error(
          `RAG collection router did not select a valid collection for org '${params.orgSlug}'`,
        );
      }
      this.logger.debug(
