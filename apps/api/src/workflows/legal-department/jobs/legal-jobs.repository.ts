...
} from '../workflows/compliance-audit/compliance-audit.types';
import {
...
function resolveWorkflowSlug(request: EnqueueJobRequest): string {
  const metadataJobType = request.metadata?.jobType;
  if (typeof metadataJobType === 'string' && metadataJobType.trim()) {
    return metadataJobType;
  }
  const data = request.data as Record<string, unknown>;
  const capabilitySlug = data.capabilitySlug;
  if (typeof capabilitySlug === 'string' && capabilitySlug.trim()) {
    return capabilitySlug;
  }
  return resolveJobType(request.metadata);
}
...
export class LegalJobsRepository {
  ...
  async insertQueued(
    ...
    const row = {
      ...
      workflow_slug: resolveWorkflowSlug(request),
