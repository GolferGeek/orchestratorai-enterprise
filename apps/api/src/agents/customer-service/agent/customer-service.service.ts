...
export class CustomerServiceService implements OnModuleInit {
  ...
  async process(input: CustomerServiceInput): Promise<CustomerServiceResult> {
    ...
    this.logger.log(
      `Starting customer service workflow: conversationId=${conversationId}, mode=${input.interactionMode || 'text'}, historyLength=${input.messages?.length || 0}`,
    );
    ...
    this.logger.log(
      `Customer service workflow completed: conversationId=${conversationId}, status=${finalState.status}, intent=${finalState.intent}, duration=${duration}ms`,
    );
  ...
  /**
   * Get status of a workflow by conversation ID
   */
  async getStatus(conversationId: string): Promise<CustomerServiceStatus | null> {
    try {
      const config = {
        configurable: {
          thread_id: conversationId,
      ...
      this.logger.error(`Failed to get status for conversation ${conversationId}:`, error);
      return null;
    }
  }
