...
// Unified platform capabilities available inside apps/web.
...
export class AuthController {
  ...
  getEntitlements(
    ...
    // Phase 2 keeps the existing Command entitlement behavior while changing
    // the targets from product applications to unified in-app capabilities.
    const products = ALL_CAPABILITIES.map((product) => ({
      ...product,
      hasAccess: true,
    }));
  ...
  @ApiOperation({
    ...
    description:
      'Called by unified platform modules to authorize an incoming request in a single round-trip. Returns 200 with principal info when allowed, 401 when the token is invalid, 403 when the permission is denied.',
  })
  ...
  @RequirePermission('admin:users')
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Send password reset email for a user (admin only)' })
  @ApiResponse({
    status: 200,
  ...
  async initiatePasswordReset(
    ...
    return this.authService.initiatePasswordReset(passwordResetDto.email);
  }
}
