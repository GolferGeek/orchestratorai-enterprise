/**
 * Ambient Dispatch Service V2
 *
 * Routes invoke requests to Ambient processing services.
 * Handles both:
...
@Injectable()
export class AmbientDispatchService {
  private readonly logger = new Logger(AmbientDispatchService.name);
  ...
  registerHandler(
    ...
    this.logger.log(`Registered Ambient handler: ${agentSlug}`);
  }
  ...
  async invoke(
    ...
    try {
      ...
      if (!handler) {
        throw new Error(`No Ambient handler for: ${context.agentSlug}. Available: ${Array.from(this.handlers.keys()).join(', ')}`);
      }
