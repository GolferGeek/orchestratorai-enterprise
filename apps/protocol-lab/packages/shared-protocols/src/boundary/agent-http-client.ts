...
export const AGENT_ENDPOINTS: Record<string, AgentEndpoint> = {
  'prairie-ridge': { baseUrl: `http://localhost:${process.env.PROTOCOL_LAB_PRAIRIE_RIDGE_PORT ?? '6407'}`, agent: 'prairie-ridge' },
  'buildwell': { baseUrl: `http://localhost:${process.env.PROTOCOL_LAB_BUILDWELL_PORT ?? '6408'}`, agent: 'buildwell' },
};
