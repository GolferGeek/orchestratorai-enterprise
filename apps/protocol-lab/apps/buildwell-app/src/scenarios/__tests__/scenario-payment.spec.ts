...
// ---------------------------------------------------------------------------
// Tests: ApexOemService Lightning payment wiring (Scenario 6)
// ---------------------------------------------------------------------------

describe('ApexOemService — Lightning L402 payment step (Scenario 6)', () => {
  beforeEach(() => {
    jest.resetAllMocks();
    ...
    global.fetch = jest.fn().mockImplementation((url: string) => {
      if (typeof url === 'string' && url.includes('/buildwell/specs/validate')) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ valid: true, specCode: 'dexos1 Gen 3' }),
        });
      }
      if (typeof url === 'string' && url.includes('/alloytech/production/schedule')) {
        return Promise.resolve({
          ok: true,
  ...
  it('calls checkProviderPreflight("lightning") before initiating payment', async () => {
    const { ApexOemService } = await import('../../apex-oem/apex-oem.service');

    // Inline import to get mock access — the module is freshly required
    const service = new ApexOemService({
  ...
  it('payment result in apexOem output contains a non-synthetic txHash', async () => {
    const { ApexOemService } = await import('../../apex-oem/apex-oem.service');
    const service = new ApexOemService({
      resolve: jest.fn().mockImplementation(() => new (LightningL402PaymentProvider as unknown as jest.Mock)()),
      resolveWith: jest.fn().mockImplementation(() => new (LightningL402PaymentProvider as unknown as jest.Mock)()),
...
describe('ScenarioService — Stripe payment step (Scenario 8)', () => {
  beforeEach(() => {
    ...
    global.fetch = jest.fn().mockImplementation((url: string) => {
      if (typeof url === 'string' && url.includes('/prairie-ridge/compliance/validate')) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ approved: true, score: 87 }),
        });
      }
      // AlloyTech Supply quality inspection — now called via HTTP after boundary hardening
      if (typeof url === 'string' && url.includes('/alloytech/quality/inspect')) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({
            batchNumber: 'BN-2026-0221',
  ...
  it('calls checkProviderPreflight("stripe") before initiating Stripe payment', async () => {
    ...
    const mockOem = {
      ...
      getSpecRequirements: jest.fn().mockReturnValue([{ id: 'spec-1', requiredSpecCode: 'dexos1 Gen 3', application: 'engine', viscosityGrade: '5W-30', operatingTempRange: { min: -40, max: 150 }, volumePerUnitMl: 6402, annualUnits: 50000, criticality: 'high' }]),
    ...
    const service = new ScenarioService(
      mockBuildwell as any,
      mockAlloytech as any,
      mockOem as any,
