...
@Injectable()
export class ApexOemService {
  private readonly dataLoader: DataLoaderService;
  private readonly security: SecurityService;
  private static readonly ORG_ID = 'apex-oem';
  ...
  async submitPurchaseOrder(poId: string, effectiveConfig?: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown }> {
    ...
    const securityEnvelope = this.security.generateEnvelope(ApexOemService.ORG_ID, rawPayload);
  ...
  async querySpecAvailability(specCode: string, effectiveConfig?: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown }> {
    ...
    const securityEnvelopeQSA = this.security.generateEnvelope(ApexOemService.ORG_ID, rawPayloadQSA);
    tracer.traceSync('After Signing', 'identity', 'oauth-jwt', () => ({
      ...rawPayloadQSA,
      ...securityEnvelopeQSA,
    }));
  ...
  async placeBid(params: {
    ...
    // Step 2: Signing (real HMAC-SHA256 signing via SecurityService)
    const securityEnvelopeBid = this.security.generateEnvelope(ApexOemService.ORG_ID, rawPayloadBid);
    tracer.traceSync('After Signing', 'identity', 'oauth-jwt', () => ({
      ...rawPayloadBid,
      ...securityEnvelopeBid,
    }));
