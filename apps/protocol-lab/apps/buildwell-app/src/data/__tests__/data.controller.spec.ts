...
/**
 * Unit tests for DataController (Buildwell app).
 *
 * These tests verify:
...
describe('DataController (buildwell)', () => {
  ...
  describe('successful responses', () => {
    it('getTransactions returns the DataFile from the loader', () => {
      const mockData = { filename: 'transactions', orgId: 'buildwell', records: [{ id: 't1' }], loadedAt: '2026-01-01' };
      __mockLoad.mockReturnValue(mockData);

      const result = controller.getTransactions('buildwell');
      expect(result).toBe(mockData);
      expect(__mockLoad).toHaveBeenCalledWith('buildwell', 'transactions');
    });

    it('getFile returns the DataFile from the loader', () => {
      const mockData = { filename: 'purchase-orders', orgId: 'apex-oem', records: [], loadedAt: '2026-01-01' };
      __mockLoad.mockReturnValue(mockData);

      const result = controller.getFile('apex-oem', 'purchase-orders');
      expect(result).toBe(mockData);
    });
