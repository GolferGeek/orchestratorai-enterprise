/**
 * Boundary Wrapper Tests — buildwell-app
 *
 * Verifies that:
 * - Scenario 10 (New OEM Onboarding) uses HTTP calls instead of in-process BuildwellService calls
 * - Scenario 8 Step 1 uses HTTP for Alloytech quality inspection
...
function makeAlloytechInspectionResponse() {
  return {
...
function makeMinimalBuildwellMocks() {
  return {
...
function makeMinimalAlloytechMock() {
  return {
    inspectQuality: jest.fn().mockReturnValue(makeAlloytechInspectionResponse()),
...
describe('buildwell-app boundary wrappers', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    ...
    mockAgentHttpClientCall.mockImplementation((path: string) => {
      if (path.includes('/alloytech/quality/inspect')) {
        return Promise.resolve(makeAlloytechInspectionResponse());
      }
      if (path.includes('/buildwell/formulations/lookup')) {
        return Promise.resolve({ formulations: makeFormulationCatalog(), qualifyingSpecifications: [] });
      }
      if (path.includes('/buildwell/formulations')) {
        return Promise.resolve(makeFormulationCatalog());
      }
      if (path.includes('/buildwell/specs/validate')) {
        return Promise.resolve({ valid: true, specCode: 'dexos1 Gen 3', meetsSpec: true, reason: 'ok' });
      }
      if (path.includes('/buildwell/pricing')) {
        return Promise.resolve([{ tierName: 'Standard', minVolumeGallons: 100, pricePerGallon: 14.5 }]);
      }
      if (path.includes('/buildwell/onboarding/start')) {
  ...
  it('Scenario 10 uses HTTP calls (not direct BuildwellService methods)', async () => {
    const mockBuildwell = makeMinimalBuildwellMocks();
    const mockAlloytech = makeMinimalAlloytechMock();
    const mockOem = makeMinimalOemMock();

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const service = new ScenarioService(mockBuildwell as any, mockAlloytech as any, mockOem as any, makeFactoryService() as any);
    await service.runScenario(10);

    // In-process buildwell service methods must NOT have been called for scenario 10 data
    expect(mockBuildwell.getFormulationCatalog).not.toHaveBeenCalled();
    expect(mockBuildwell.lookupFormulation).not.toHaveBeenCalled();
    expect(mockBuildwell.validateSpec).not.toHaveBeenCalled();
    expect(mockBuildwell.getPricingTiers).not.toHaveBeenCalled();
    expect(mockBuildwell.startOnboarding).not.toHaveBeenCalled();

    // AgentHttpClient.call must have been used for all data lookups
    expect(mockAgentHttpClientCall).toHaveBeenCalledWith('/buildwell/formulations');
    expect(mockAgentHttpClientCall).toHaveBeenCalledWith('/buildwell/formulations/lookup', 'POST', { specCode: 'dexos1 Gen 3' });
    expect(mockAgentHttpClientCall).toHaveBeenCalledWith('/buildwell/specs/validate', 'POST', { specCode: 'dexos1 Gen 3', productId: 'form-001' });
    expect(mockAgentHttpClientCall).toHaveBeenCalledWith('/buildwell/pricing?productId=form-001');
    expect(mockAgentHttpClientCall).toHaveBeenCalledWith('/buildwell/onboarding/start', 'POST', {
      companyName: 'Rivian Automotive LLC',
      partnerCode: 'RIVIAN-NEW',
    });
  });

  it('Scenario 8 step 1 uses HTTP for Alloytech quality inspection', async () => {
    const mockBuildwell = makeMinimalBuildwellMocks();
    const mockAlloytech = makeMinimalAlloytechMock();
    const mockOem = makeMinimalOemMock();
    ...
    // In-process AlloytechService.inspectQuality must NOT have been called
    expect(mockAlloytech.inspectQuality).not.toHaveBeenCalled();

    // AgentHttpClient must have been called for the alloytech quality endpoint
    expect(mockAgentHttpClientCall).toHaveBeenCalledWith(
      '/alloytech/quality/inspect',
      'POST',
      { batchNumber: 'BN-2026-0221' },
    );
  });
  ...
  it('Strict boundary mode enforcement: assertCrossAgentBoundary is called', async () => {
    ...
    await service.runScenario(8);
    expect(mockAssertCrossAgentBoundary).toHaveBeenCalledWith('buildwell-scenario', 'alloytech');

    // Run scenario 10 — assertCrossAgentBoundary should be called at entry
    jest.clearAllMocks();
    mockAgentHttpClientCall.mockImplementation((path: string) => {
      if (path.includes('/buildwell/formulations/lookup')) return Promise.resolve({ formulations: [], qualifyingSpecifications: [] });
      if (path.includes('/buildwell/formulations')) return Promise.resolve([]);
      if (path.includes('/buildwell/specs/validate')) return Promise.resolve({ valid: true, specCode: 'dexos1 Gen 3', meetsSpec: true, reason: 'ok' });
      if (path.includes('/buildwell/pricing')) return Promise.resolve([]);
      if (path.includes('/buildwell/onboarding/start')) return Promise.resolve({ companyName: 'Rivian', partnerCode: 'RIVIAN-NEW', steps: [], totalEstimatedDays: 0, initiatedAt: new Date().toISOString() });
      return Promise.resolve({});
    });

    await service.runScenario(10);
    expect(mockAssertCrossAgentBoundary).toHaveBeenCalledWith('apex-oem', 'buildwell');
  });
});
