...
onMounted(async () => {
  try {
    await Promise.all([
      buildwellStore.fetchAll(),
      alloytechStore.fetchAll(),
      oemStore.fetchAll(),
    ]);
  } catch (e) {
...
<template>
  <div class="flex flex-col h-full bg-gray-950 text-gray-100">
    ...
    <div v-if="loadError" class="px-4 py-2 bg-red-950/50 border-b border-red-900/50 text-xs text-red-400">
      Backend load error: {{ loadError }} — ensure the Buildwell backend is running on port 6408
    </div>
    ...
    <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div class="flex gap-3 p-3 h-[42%] min-h-0">
        ...
        <div class="flex-1 min-w-0">
          <OrgPanel
            org-name="BuildWell Manufacturing"
            org-id="buildwell"
            role="Lubricant Manufacturer"
            identity-provider="did"
            :trust-score="buildwellStore.trustScore"
            :trust-level="buildwellStore.trustLevel"
            :circuit-breaker-state="buildwellStore.circuitBreakerState"
            :messages="buildwellStore.messages"
            accent-color="amber"
            :loading="buildwellStore.loading"
          />
        </div>

        <div class="flex-1 min-w-0">
          <OrgPanel
            org-name="AlloyTech Supply"
            org-id="alloytech"
            role="Contract Manufacturer"
            identity-provider="local-keys"
            :trust-score="alloytechStore.trustScore"
            :trust-level="alloytechStore.trustLevel"
            :circuit-breaker-state="alloytechStore.circuitBreakerState"
            :messages="alloytechStore.messages"
            accent-color="green"
            :loading="alloytechStore.loading"
          />
        </div>
      </div>
