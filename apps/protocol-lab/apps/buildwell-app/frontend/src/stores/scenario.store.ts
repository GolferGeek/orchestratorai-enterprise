...
import { useBuildwellStore } from './buildwell.store';
import { useAlloytechStore } from './alloytech.store';
import { useApexOemStore } from './apex-oem.store';
    ...
    steps.forEach((step) => {
      ...
      if (scenarioId === 6) {
        if (step.stepNumber <= 3) {
          oemStore.addMessage({ ...msg, direction: 'outbound', from: 'OEM', to: 'BuildWell Manufacturing' });
        } else if (step.stepNumber <= 6) {
          buildwellStore.addMessage({ ...msg, direction: 'inbound', from: 'OEM', to: 'BuildWell Manufacturing' });
        } else {
          alloytechStore.addMessage({ ...msg, direction: 'inbound', from: 'BuildWell Manufacturing', to: 'AlloyTech Supply' });
        }
        ...
        if (step.stepNumber <= 4) {
          alloytechStore.addMessage({ ...msg, direction: 'outbound', from: 'AlloyTech Supply', to: 'BuildWell Manufacturing' });
        } else if (step.stepNumber <= 7) {
          buildwellStore.addMessage({ ...msg, direction: 'inbound', from: 'AlloyTech Supply', to: 'BuildWell Manufacturing' });
        } else {
          oemStore.addMessage({ ...msg, direction: 'inbound', from: 'BuildWell Manufacturing', to: 'OEM' });
        }
        ...
        if (step.layer === 'trust' || step.provider === 'reputation' || step.provider === 'first-contact') {
          buildwellStore.addMessage({ ...msg, direction: 'inbound', from: 'OEM-New', to: 'BuildWell Manufacturing' });
          oemStore.addMessage({ ...msg, direction: 'outbound', from: 'OEM-New', to: 'BuildWell Manufacturing' });
        } else if (step.stepNumber <= 6) {
          oemStore.addMessage({ ...msg, direction: 'outbound', from: 'OEM-New', to: 'BuildWell Manufacturing' });
        } else {
          buildwellStore.addMessage({ ...msg, direction: 'inbound', from: 'OEM-New', to: 'BuildWell Manufacturing' });
