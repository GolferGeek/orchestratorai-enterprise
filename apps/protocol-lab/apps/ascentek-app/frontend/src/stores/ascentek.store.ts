...
import { ref } from 'vue';
import { ascentekApi, transactionsApi } from '@/services/api';
...
export const useAscentekStore = defineStore('ascentek', () => {
  ...
  async function fetchAll(): Promise<void> {
    ...
    try {
      const [f, s, p, pa, tx] = await Promise.all([
        ascentekApi.getFormulations(),
        ascentekApi.getSpecs(),
        ascentekApi.getPricing(),
        ascentekApi.getPartners(),
        transactionsApi.getAscentekTransactions().catch(() => ({ records: [] })),
      ]);
