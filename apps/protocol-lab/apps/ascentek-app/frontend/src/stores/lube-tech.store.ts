...
import { ref } from 'vue';
import { lubeTechApi, transactionsApi } from '@/services/api';
import type { OrgMessage } from './ascentek.store';

export const useLubeTechStore = defineStore('lubeTech', () => {
  ...
  async function fetchAll(): Promise<void> {
    ...
    try {
      const [p, i, b, qs, tx] = await Promise.all([
        lubeTechApi.getProduction(),
        lubeTechApi.getInventory(),
        lubeTechApi.getBatches(),
        lubeTechApi.getQualityStandards(),
        transactionsApi.getLubeTechTransactions().catch(() => ({ records: [] })),
      ]);
      production.value = p;
      inventory.value = i;
      batches.value = b;
