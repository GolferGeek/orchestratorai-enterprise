...
/**
 * Unit tests for DataController (SunStream app).
 *
 * These tests verify:
...
describe('DataController (sunstream)', () => {
  ...
  describe('successful responses', () => {
    it('getTransactions returns the DataFile from the loader', () => {
      const mockData = { filename: 'transactions', orgId: 'sunstream', records: [{ id: 't1' }], loadedAt: '2026-01-01' };
      __mockLoad.mockReturnValue(mockData);

      const result = controller.getTransactions('sunstream');
      expect(result).toBe(mockData);
      expect(__mockLoad).toHaveBeenCalledWith('sunstream', 'transactions');
    });

    it('getFile returns the DataFile from the loader', () => {
      const mockData = { filename: 'loans', orgId: 'fcs-financial', records: [], loadedAt: '2026-01-01' };
      __mockLoad.mockReturnValue(mockData);

      const result = controller.getFile('fcs-financial', 'loans');
      expect(result).toBe(mockData);
    });
