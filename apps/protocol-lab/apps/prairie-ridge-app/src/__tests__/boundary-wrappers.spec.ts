/**
 * Boundary Wrapper Tests — prairie-ridge-app
 *
 * Verifies that Scenario 5 (New Association Onboarding) uses HTTP calls
 * through the prairieRidge controller endpoints rather than in-process service calls.
 */
...
function makeAssociations() {
  return [
    { id: 'assoc-001', name: 'AgriServ Financial', region: 'Midwest', type: 'Farm Credit Association' },
  ];
}
...
describe('prairie-ridge-app boundary wrappers', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    ...
    mockAgentHttpClientCall.mockImplementation((path: string) => {
      if (path.includes('/prairie-ridge/services')) {
        return Promise.resolve(makeServiceCatalog());
      }
      if (path.includes('/prairie-ridge/associations')) {
  ...
  it('Scenario 5 uses HTTP calls (not direct service methods)', async () => {
    const mockPrairieRidge = makeMinimalServices();
    const mockFcs = {
    ...
    const service = new ScenarioService(mockPrairieRidge as any, mockFcs as any, mockCentralFarmBank as any, makeFactoryService() as any);
    await service.runScenario(5);

    // The in-process service methods for data retrieval must NOT have been called
    expect(mockPrairieRidge.getServiceCatalog).not.toHaveBeenCalled();
    expect(mockPrairieRidge.getAssociations).not.toHaveBeenCalled();

    // AgentHttpClient.call must have been used for the data lookups
    expect(mockAgentHttpClientCall).toHaveBeenCalledWith('/prairie-ridge/services');
    expect(mockAgentHttpClientCall).toHaveBeenCalledWith('/prairie-ridge/associations');
  });
  ...
  it('Strict boundary mode catches in-process cross-agent calls', () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    new ScenarioService(mockPrairieRidge as any, mockFcs as any, mockCentralFarmBank as any, makeFactoryService() as any);
    ...
    expect(() => mockAssertCrossAgentBoundary('new-association', 'prairie-ridge')).toThrow(
      'Strict boundary violation: new-association attempted in-process call to prairie-ridge',
    );
  });

  it('HTTP wrapper preserves response shape from prairieRidge endpoints', async () => {
    const mockPrairieRidge = makeMinimalServices();
    const mockFcs = {
      getLoanApplications: jest.fn().mockReturnValue([]),
      getRateSheet: jest.fn().mockReturnValue([]),
