...
@Injectable()
export class CentralFarmBankService {
  private readonly dataLoader: DataLoaderService;
  private static readonly ORG_ID = 'central-farm-bank';
  ...
  async performOversightReview(params: {
    ...
    const payload = tracer.traceSync('Raw Payload', 'data', 'raw', () => ({
      ...
      examinerAuthority: 'Central Farm Bank District',
    }));

    // Step 2: x509 mutual authentication
    const signed = tracer.traceSync('x509 Mutual Auth', 'identity', 'x509', () => ({
      ...payload,
      certificate: 'CN=Central Farm Bank,O=Farm Credit System,C=US',
    ...
    const encrypted = tracer.traceSync('TLS-Mutual Encryption', 'encryption', 'tls-mutual', () => ({
      ...
      clientCert: 'CN=Central Farm Bank',
      serverCert: 'CN=Prairie Ridge Credit',
