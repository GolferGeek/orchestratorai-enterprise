...
import { useAgriservStore } from '@/stores/agriserv.store';
import { usePrairieRidgeStore } from '@/stores/prairie-ridge.store';
import { useCentralFarmBankStore } from '@/stores/central-farm-bank.store';
import { useTimelineStore } from '@/stores/timeline.store';
import { useHelpStore } from '@/stores/help.store';

const agriservStore = useAgriservStore();
const prairieRidgeStore = usePrairieRidgeStore();
const centralFarmBankStore = useCentralFarmBankStore();
...
const { loans, borrowers, circuitBreakerPrairieRidge: fcsCB, trustTowardPrairieRidge: fcsTrust } = storeToRefs(agriservStore);
const { services, associations, circuitBreakerFcs, trustTowardFcs } = storeToRefs(prairieRidgeStore);
const { ratings, circuitBreakerPrairieRidge: centralFarmBankCB, trustTowardPrairieRidge: centralFarmBankTrust } = storeToRefs(centralFarmBankStore);
...
const fcsMessages = computed(() =>
  messages.value
    .filter((m) => m.source.includes('agriserv') || m.target.includes('agriserv'))
    .slice(0, 5)
    .map((m) => ({
      id: m.id,
      method: m.method,
...
const prairieRidgeMessages = computed(() =>
  messages.value
    .filter((m) => m.source.includes('prairie-ridge') || m.target.includes('prairie-ridge'))
    .slice(0, 5)
    .map((m) => ({
      id: m.id,
      method: m.method,
...
const centralFarmBankMessages = computed(() =>
  messages.value
    .filter((m) => m.source.includes('central-farm-bank') || m.target.includes('central-farm-bank'))
    .slice(0, 5)
    .map((m) => ({
      id: m.id,
      method: m.method,
...
const fcsCircuitBreaker = computed(() => {
  const lastMsg = messages.value.find((m) => m.source.includes('agriserv'));
...
const centralFarmBankCircuitBreaker = computed(() => {
  const lastMsg = messages.value.find((m) => m.source.includes('central-farm-bank'));
  if (!lastMsg) return centralFarmBankCB.value;
  const cbStep = lastMsg.pipelineTrace?.steps?.find(
    (s) => s.layer === 'resilience' && s.provider === 'circuit-breaker'
  );
  const state = (cbStep?.metadata as Record<string, unknown>)?.state as string | undefined;
  if (state === 'CLOSED' || state === 'HALF-OPEN' || state === 'OPEN') return state;
  return centralFarmBankCB.value;
});
...
const centralFarmBankTrustComputed = computed(() => {
  const lastMsg = messages.value.find((m) => m.source.includes('central-farm-bank'));
  if (!lastMsg) return centralFarmBankTrust.value;
  const trustStep = lastMsg.pipelineTrace?.steps?.find((s) => s.layer === 'trust');
  if (!trustStep?.metadata) return centralFarmBankTrust.value;
  const meta = trustStep.metadata as Record<string, unknown>;
  const score = typeof meta.trustScore === 'number' ? meta.trustScore : centralFarmBankTrust.value.score;
  const level = typeof meta.trustLevel === 'string' ? meta.trustLevel : centralFarmBankTrust.value.level;
  return { level: level as typeof centralFarmBankTrust.value.level, score };
});
...
onMounted(async () => {
  try {
    await Promise.all([
      agriservStore.loadData(),
      prairieRidgeStore.loadData(),
      centralFarmBankStore.loadData(),
    ]);
  } catch (e) {
...
<template>
  <div class="flex flex-col h-full bg-slate-950 text-slate-100">
    ...
    <div v-if="loadError" class="px-4 py-2 bg-red-950/50 border-b border-red-900/50 text-xs text-red-400">
      Backend load error: {{ loadError }} — ensure the Prairie Ridge Credit backend is running on port 6407
    </div>
      ...
      <div class="flex gap-3 p-3 h-[42%] min-h-0">
        ...
        <div class="flex-1 min-w-0">
          <OrgPanel
            ...
            circuit-breaker-target="prairie-ridge"
        ...
        <!-- Prairie Ridge Credit (center) — highlighted -->
        <div class="flex-1 min-w-0">
          <OrgPanel
            org-id="prairie-ridge"
            ...
            :recent-messages="prairieRidgeMessages"
            :data-count="services.length + associations.length"
            data-label="services + associations"
          />
        </div>

        <!-- Central Farm Bank (right) -->
        <div class="flex-1 min-w-0">
          <OrgPanel
            org-id="central-farm-bank"
            :trust-level="centralFarmBankTrustComputed.level"
            :trust-score="centralFarmBankTrustComputed.score"
            :circuit-breaker-state="centralFarmBankCircuitBreaker"
            circuit-breaker-target="prairie-ridge"
            :recent-messages="centralFarmBankMessages"
