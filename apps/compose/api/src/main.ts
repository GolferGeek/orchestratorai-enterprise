...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('Compose API')
    .setDescription(
      'Compose API — simple composable agents (context, RAG, API, external, media runners)',
    )
    .setVersion('1.0')
    .addBearerAuth(
      {
  ...
  // Compose API port: COMPOSE_API_PORT (preferred) or API_PORT (fallback)
  // Dev: 6300, Prod: 7300
  const portEnv = process.env.COMPOSE_API_PORT || process.env.API_PORT;
  if (!portEnv) {
    throw new Error(
      'COMPOSE_API_PORT environment variable is required. ' +
        'Set COMPOSE_API_PORT in your .env file (Dev: 6300, Prod: 7300)',
    );
  }
