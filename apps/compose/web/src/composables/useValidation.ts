...
// Sanitization is handled server-side in Compose; these are identity-pass stubs.
...
function sanitizeWithProfile(value: unknown, options?: SanitizationOptions): SanitizationResult {
  return { sanitized: value, wasModified: false, profile: options?.profile || 'moderate' };
}

const SanitizationHelpers = {
  forApiInput: (value: string): string => value,
  forSearch: (value: string): string => value,
  forRichText: (value: string): string => value,
};
