...
const reviewContext = computed<ExecutionContextLike | null>(() => {
  ...
  return {
    ...
    agentType: 'langgraph',
    provider: memoProvider.value ?? 'anthropic',
    model: memoModel.value ?? 'claude-sonnet-4-6',
  };
});

const parentLink = computed(() =>
  `/app/workflows/legal-department/due-diligence?jobId=${encodeURIComponent(parentJobId.value)}`,
);
