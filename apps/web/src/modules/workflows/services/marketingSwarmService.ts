 ...
 * - Starting swarm executions via the Workflows Marketing Swarm endpoint
 * - Real-time progress via observability SSE stream
 * - Fetching results
 *
 * Executions go through the Workflows module; progress is delivered by
 * the observability SSE stream.
 */
...
const MARKETING_SWARM_API_PREFIX = '/workflows/marketing-swarm';
...
class MarketingSwarmService {
   ...
   *
   * Creates a workflow conversation id and initializes ExecutionContext.
   *
  ...
  async startSwarmExecution(
    ...
    try {
      ...
      console.log('[MarketingSwarm] Starting workflow execution', {
