<template>
  ...
  <div class="detail-view">
    ...
    <div class="detail-body">
      <div v-if="loading || (!rbacStore.isInitialized && allUsers.length === 0)" class="loading-state">
      ...
      <div v-else class="master-detail-container">
        ...
        <div class="master-panel">
          ...
          <ion-card v-if="organizations.length > 1" class="org-selector-card">
            <ion-card-content>
              <div class="org-label-container">
                <ion-label color="medium">Role Scope</ion-label>
              </div>
          ...
          <div class="action-buttons">
            ...
            <ion-button
              ...
              @click="showAddUserToOrgForm = true"
            >
            ...
            <template v-if="users.length > 0">
              ...
              >
                ...
                <ion-label>
                  ...
                  <p>{{ user.email }}</p>
                  <p v-if="getUserMemberships(user.id).length > 0" class="orgs-list">
                    <ion-badge
                      v-for="membership in getUserMemberships(user.id).slice(0, 3)"
                      :key="`${user.id}-${membership.organizationSlug}`"
                      color="medium"
                      class="org-badge"
                    >
                    ...
                    <ion-badge
                      v-if="getUserMemberships(user.id).length > 3"
                      color="medium"
                      class="org-badge"
                    >
                      +{{ getUserMemberships(user.id).length - 3 }} more
                    </ion-badge>
                  </p>
                  <p v-else class="orgs-list">
                    <ion-badge color="medium" class="org-badge empty-org-badge">
                      No organizations
                    </ion-badge>
                  </p>
        ...
        <div class="detail-panel" v-if="selectedUser">
          ...
          <ion-card class="memberships-card">
            <ion-card-header>
              <div class="card-header-row">
                <div>
                  <ion-card-title>Organization Memberships</ion-card-title>
                  <ion-card-subtitle>Organizations and roles for this user</ion-card-subtitle>
                </div>
                <ion-button
                  fill="outline"
                  size="small"
            ...
            <ion-card-content>
              <ion-list v-if="selectedUserMemberships.length > 0" class="membership-list">
                <ion-item v-for="membership in selectedUserMemberships" :key="membership.organizationSlug">
                  <ion-label>
                    ...
                    <p class="roles-list">
                      <ion-badge
                        ...
                        :key="`${membership.organizationSlug}-${role.name}`"
                        :color="getRoleBadgeColor(role.name)"
                        class="role-badge"
                      >
                        {{ role.displayName }}
                  ...
                  <ion-button
                    ...
                    size="small"
                    color="medium"
                    @click="confirmRemoveFromMembership(membership)"
                  >
                    Remove
                  </ion-button>
                </ion-item>
              </ion-list>
              <p v-else class="empty-message">No organization memberships</p>
            </ion-card-content>
          </ion-card>
          ...
          <ion-card class="roles-card">
            <ion-card-header>
              <ion-card-title>Current Scope Roles</ion-card-title>
              <ion-card-subtitle>Roles for the active role scope</ion-card-subtitle>
            </ion-card-header>
            <ion-card-content>
              <div class="roles-grid">
                ...
                <p v-if="!selectedUser.roles || selectedUser.roles.length === 0" class="empty-message">
                  No roles assigned in the active role scope
                </p>
              </div>
            </ion-card-content>
          </ion-card>
      ...
      <!-- Add Selected User to Organization Modal -->
      <ion-modal :is-open="showAddUserToOrgForm" @didDismiss="resetAddUserToOrgForm">
        <ion-header>
          <ion-toolbar>
            <ion-title>Add {{ selectedUser?.displayName || selectedUser?.email }} to Organization</ion-title>
            <ion-buttons slot="end">
          ...
          <ion-card>
            <ion-card-content>
              <ion-item>
                ...
                <ion-select 
                  v-model="selectedMembershipToAdd.organizationSlug" 
                  interface="popover"
                  placeholder="Choose an organization"
                >
                  <ion-select-option 
                    v-for="org in availableOrganizationsToAdd" 
                    :key="org.slug" 
                    :value="org.slug"
                  >
                    {{ org.name }}
                ...
                <p v-if="availableOrganizationsToAdd.length === 0" class="form-hint">
              ...
              <ion-item>
                ...
                <ion-select 
                  ...
                  v-model="selectedMembershipToAdd.role" 
                  interface="popover"
              ...
              <ion-button
                expand="block"
                @click="addSelectedUserToOrganization"
                :disabled="!selectedUser || !selectedMembershipToAdd.organizationSlug || !selectedMembershipToAdd.role || addingUserToOrg || availableOrganizationsToAdd.length === 0"
                class="create-button"
              >
                <ion-spinner v-if="addingUserToOrg" slot="start"></ion-spinner>
                <ion-icon v-else :icon="personAddOutline" slot="start"></ion-icon>
                {{ addingUserToOrg ? 'Adding...' : 'Add to Organization' }}
              </ion-button>
            </ion-card-content>
          </ion-card>
        </ion-content>
...
import {
  ...
  mailOutline, personCircleOutline, personAddOutline
} from 'ionicons/icons';
...
const selectedUserMemberships = computed(() => {
  if (!selectedUser.value) return [];
  return organizationMemberships.value.get(selectedUser.value.id) ?? [];
});

function getUserMemberships(userId: string): UserMembership[] {
  return organizationMemberships.value.get(userId) ?? [];
}

// Admin Users is a user directory first. RBAC membership adds role details
// for the selected organization when that data is available.
const users = computed(() => {
  if (allUsers.value.length > 0) {
    const globalUsersById = new Map(
      rbacStore.currentOrgUsers.map((user) => [user.userId, user]),
    );

    return allUsers.value.map((user) => {
      const globalUser = globalUsersById.get(user.id);
      return {
        id: user.id,
        email: user.email,
...
const availableOrganizationsToAdd = computed(() => {
  if (!selectedUser.value) return [];
  const existing = new Set(selectedUserMemberships.value.map((membership) => membership.organizationSlug));
  return allOrganizations.value.filter((org) => !existing.has(org.slug));
});
...
onMounted(async () => {
  ...
  try {
    ...
    console.error('Failed to initialize RBAC user details:', error);
  }
...
async function loadAllMemberships() {
  ...
  for (const org of allOrganizations.value) {
    try {
      ...
      console.error(`Failed to load users for organization ${org.slug}:`, error);
    }
  }
  organizationMemberships.value = membershipMap;
  syncSelectedUserFromDirectory();
}

function selectFirstUserIfNeeded() {
  if (selectedUser.value || users.value.length === 0) return;
  void selectUser(users.value[0]);
}

function syncSelectedUserFromDirectory() {
  if (!selectedUser.value) return;

  const currentUser = users.value.find((user) => user.id === selectedUser.value?.id);
  if (currentUser) {
...
async function selectUser(user: UserWithRoles) {
  ...
  selectedMembershipToAdd.value = { organizationSlug: '', role: 'member' };
...
async function addSelectedUserToOrganization() {
  if (!selectedUser.value || !selectedMembershipToAdd.value.organizationSlug || !selectedMembershipToAdd.value.role) return;
  
  addingUserToOrg.value = true;
  try {
    await rbacService.assignRole(
      selectedUser.value.id,
      selectedMembershipToAdd.value.organizationSlug,
      selectedMembershipToAdd.value.role
    );
...
async function confirmRemoveFromMembership(membership: UserMembership) {
  if (!selectedUser.value) return;

  const alert = await alertController.create({
    header: 'Remove from Organization',
    message: `Remove ${selectedUser.value.displayName || selectedUser.value.email} from "${membership.organizationName}"? This will remove all roles in that organization.`,
    buttons: [
      { text: 'Cancel', role: 'cancel' },
      {
        text: 'Remove',
        role: 'destructive',
        handler: () => removeUserMembership(membership),
      },
    ],
  });
  await alert.present();
...
async function removeUserMembership(membership: UserMembership) {
  if (!selectedUser.value) return;
...
.users-list ion-item.selected-user ion-label {
  color: white !important;
}

.users-list ion-item.selected-user ion-label h2,
.users-list ion-item.selected-user ion-label p,
.users-list ion-item.selected-user ion-avatar,
.users-list ion-item.selected-user .avatar-placeholder {
  color: white !important;
}
