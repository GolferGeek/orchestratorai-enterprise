...
// Stub types for legacy methods that are no longer used in Admin Web
// These allow the shared auth infrastructure to compile without the deleted type files
type TaskResponse = unknown;
type AgentInfo = unknown;
type LLMSelection = unknown;
type SendMessageRequest = { content: string; llmSelection?: unknown };
type SendMessageResponse = unknown;
type AgentHierarchyResponse = unknown;
import {
  getSecureApiBaseUrl,
  getSecureHeaders,
...
// useApiSanitization removed (admin web does not handle agent messages)
...
class ApiService {
  ...
  async postTaskToOrchestrator(
    ...
    // No sanitization in Admin Web (no agent message handling)
