...
class TokenStorage {
  ...
  /**
   * Get access token (synchronous — returns whatever is in memory right now).
   * Only use this in synchronous contexts such as Axios request interceptors.
   * For all other callers, prefer the async getAccessToken() which ensures
   * the storage is initialized before returning.
   */
