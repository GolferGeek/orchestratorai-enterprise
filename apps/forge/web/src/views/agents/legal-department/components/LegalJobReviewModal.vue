            ...
            <template v-if="reviewMode === 'review'">
              ...
              <section class="section">
                ...
                <div class="research-decision-buttons">
                  <ion-button
                    color="success"
                    :disabled="submitting || !props.context"
...
// ────────────────────────────────────────────────────────────────────────
// Research HITL mode state (Deepen / Redirect — Phase 4 Step 4.8)
// ────────────────────────────────────────────────────────────────────────

/** Current HITL sub-mode for research jobs. */
const reviewMode = ref<'review' | 'deepen' | 'redirect'>('review');

/** Node IDs checked in deepen (multi) or redirect (single) mode. */
const selectedNodeIds = ref<string[]>([]);

/** Optional free-text guidance for Deepen submissions. */
const deepenGuidance = ref('');

/** Newline-separated replacement questions for Redirect submissions. */
...
watch(
  ...
  async ([open, id]) => {
    if (!open || !id) {
      ...
      // Clear research HITL mode state on close
      reviewMode.value = 'review';
      selectedNodeIds.value = [];
      deepenGuidance.value = '';
      replacementQuestions.value = '';
...
/* ── Research HITL decision buttons ── */
