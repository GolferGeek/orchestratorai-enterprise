  ...
  function setActiveConversation(conversationId: string | null): void {
    ...
    if (conversationId) {
      ...
      if (conversation) {
        ...
        const orgSlug = agentOrgSlug || authStore.currentOrganization;

        // Get LLM selection (provider/model)
        const provider = llmPreferencesStore.selectedProvider || 'ollama';
        const model = llmPreferencesStore.selectedModel || 'llama3.2:1b';
