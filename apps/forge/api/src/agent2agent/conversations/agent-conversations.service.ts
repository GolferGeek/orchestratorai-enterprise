...
export class AgentConversationsService {
   ...
   * Calls the internal MarketingSwarmService directly (no HTTP).
   */
  private async cleanupMarketingSwarmData(
    ...
    try {
      ...
      const taskRows = (tasks ?? []) as Array<{ id: string }>;
      ...
      for (const task of taskRows) {
        try {
          ...
          const success = await this.marketingSwarmService.deleteTask(task.id);
          if (success) {
            this.logger.log(
