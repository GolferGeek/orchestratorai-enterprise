/**
 * Mutual NDA fixture for contract-review E2E tests.
 *
 * This NDA is intentionally written with a mix of standard and problematic
 * clauses so that specialists have material to flag. The clause structure
 * is consistent enough that segmentation should reliably parse it.
 *
 * Known risk areas (expected specialist findings):
 * - Section 3: One-sided indemnification (high risk)
 * - Section 5: Overly broad IP assignment (high risk)
 * - Section 7: Unlimited liability (critical risk)
 * - Section 9: Unilateral termination right (medium risk)
 * - Section 11: Mandatory arbitration with class action waiver (medium risk)
 */

export const MUTUAL_NDA_TEXT = `MUTUAL NON-DISCLOSURE AGREEMENT
...
/** Expected number of identifiable clauses in the NDA (approximate — segmentation may vary) */
export const MUTUAL_NDA_EXPECTED_SECTION_COUNT = 12;

/** Known high/critical risk sections that specialists should flag */
export const MUTUAL_NDA_EXPECTED_RISKS = {
  indemnification: {
    description: 'One-sided indemnification (Widget indemnifies Acme but not vice versa)',
    expectedMinRisk: 'high' as const,
    article: 3,
  },
  ipAssignment: {
    description: 'Overly broad IP assignment (all Widget IP becomes Acme property, even unrelated)',
    expectedMinRisk: 'high' as const,
    article: 5,
  },
  unlimitedLiability: {
    description: 'Unlimited liability for Widget, $100 cap for Acme',
    expectedMinRisk: 'critical' as const,
    article: 7,
  },
  unilateralTermination: {
    description: 'Acme can terminate for any reason, Widget only for material breach with 90-day cure',
    expectedMinRisk: 'medium' as const,
    article: 9,
  },
  classActionWaiver: {
    description: 'Mandatory arbitration with class action waiver',
    expectedMinRisk: 'medium' as const,
    article: 11,
  },
};
