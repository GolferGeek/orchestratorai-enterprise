...
export class MessageLoggingInterceptor implements NestInterceptor {
  ...
  constructor(agentId: string, protocolApiPort = 4000) {
  ...
  private extractSource(req: Request): string {
    ...
    if (referer) {
      if (referer.includes(':4001')) return 'research-hub';
      if (referer.includes(':4002')) return 'market-pulse';
      if (referer.includes(':4003')) return 'content-forge';
      if (referer.includes(':4010')) return 'frontend';
    }
