...
const orgColorClass = computed(() => {
  switch (props.orgId) {
    case 'fcs': return 'border-blue-500/50 bg-blue-950/20';
    case 'sunstream': return 'border-emerald-500/50 bg-emerald-950/20';
    case 'agribank': return 'border-amber-500/50 bg-amber-950/20';
  }
});

const headerColorClass = computed(() => {
  switch (props.orgId) {
    case 'fcs': return 'text-blue-400';
    case 'sunstream': return 'text-emerald-400';
    case 'agribank': return 'text-amber-400';
  }
});

const identityBadgeColor = computed(() => {
  switch (props.orgId) {
    case 'fcs': return 'bg-blue-900/60 text-blue-300 border-blue-700';
    case 'sunstream': return 'bg-emerald-900/60 text-emerald-300 border-emerald-700';
    case 'agribank': return 'bg-amber-900/60 text-amber-300 border-amber-700';
  }
});

const messageSourceColor = (source: string) => {
  if (source.includes('fcs')) return 'text-blue-400';
  if (source.includes('sunstream')) return 'text-emerald-400';
  if (source.includes('agribank')) return 'text-amber-400';
