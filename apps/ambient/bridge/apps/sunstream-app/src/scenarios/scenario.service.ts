...
import { SunstreamService } from '../sunstream/sunstream.service';
import { FcsFinancialService } from '../fcs-financial/fcs-financial.service';
import { AgribankService } from '../agribank/agribank.service';
...
export class ScenarioService {
  ...
  constructor(
    private readonly sunstream: SunstreamService,
    private readonly fcs: FcsFinancialService,
    private readonly agribank: AgribankService,
  ...
  private postScenarioMessage(
    ...
    const message: ProtocolMessage = {
      id: messageId,
      timestamp: now,
      source: trace?.source || 'sunstream-ecosystem',
      target: trace?.target || 'sunstream',
  ...
  private async scenario1_loanCompliance(effectiveConfig: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown; messageId: string }> {
    ...
    this.recordTransaction('fcs-financial', {
      messageId,
      type: 'loan-application',
      sourceAgent: 'fcs-financial',
      targetAgent: 'sunstream',
      ...
      status: 'success',
      sourceData: {
        file: 'fcs-financial/loan-applications',
  ...
  private async scenario2_helpdeskTicket(effectiveConfig: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown; messageId: string }> {
    ...
    this.recordTransaction('fcs-financial', {
      ...
      status: 'success',
      sourceData: {
        file: 'fcs-financial/rate-sheet',
        recordId: rateSheet.length > 0 ? rateSheet[0].id : 'rate-4001',
        recordType: 'RateSheet',
      },
      summary: 'Helpdesk ticket — Cornerstone login/timeout issue reported to SunStream',
    });
    return { ...outcome, messageId };
  }
  ...
  private async scenario3_quarterlyOversight(effectiveConfig: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown; messageId: string }> {
    void effectiveConfig; // config plumbing ready; no payment provider in this scenario
    const messageId = randomUUID();
    const criteria = this.agribank.getExaminationCriteria();
    const outcome = await this.agribank.performOversightReview({ quarter: 'Q1-2026' });
    this.recordTransaction('agribank', {
      messageId,
      type: 'examination',
      sourceAgent: 'agribank',
      targetAgent: 'sunstream',
      ...
      status: 'success',
      sourceData: {
        file: 'agribank/examination-criteria',
        recordId: criteria.length > 0 ? criteria[0].id : 'exam-A001',
        recordType: 'ExaminationCriterion',
      },
      summary: 'Quarterly oversight review Q1-2026 — AgriBank examines FCS Financial',
    });
    return { ...outcome, messageId };
  }
  ...
  private async scenario4_stressTest(effectiveConfig: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown; messageId: string }> {
    void effectiveConfig; // config plumbing ready; no payment provider in this scenario
    const messageId = randomUUID();
    const capitalRequirements = this.agribank.getCapitalRequirements();
    const outcome = await this.agribank.runStressTest({ scenario: 'adverse' });
    this.recordTransaction('agribank', {
      messageId,
      type: 'examination',
      sourceAgent: 'agribank',
      ...
      status: 'success',
      sourceData: {
        file: 'agribank/capital-requirements',
        recordId: capitalRequirements.length > 0 ? capitalRequirements[0].id : 'cap-B001',
        recordType: 'CapitalRequirement',
      },
      summary: 'Capital adequacy stress test (adverse scenario) — AgriBank via SunStream',
    });
    return { ...outcome, messageId };
  }
  ...
  private async scenario5_newAssociationOnboarding(effectiveConfig: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown; messageId: string }> {
    void effectiveConfig; // config plumbing ready; no payment provider in this scenario
    // Scenario 5 simulates a new-association caller contacting sunstream-app via HTTP.
    // All data lookups go through the sunstream HTTP endpoints, not in-process.
    assertCrossAgentBoundary('new-association', 'sunstream');

    const messageId = randomUUID();
    const sunsClient = new AgentHttpClient(AGENT_ENDPOINTS['sunstream']);
    ...
    // Step 1: Discovery — new association discovers SunStream via well-known
    tracer.traceSync('Discovery via Well-Known', 'discovery', 'well-known', () => ({
      discoveredAgent: 'sunstream',
      agentCardUrl: 'http://localhost:4007/.well-known/agent.json',
    ...
    const serviceCatalog = await sunsClient.call('/sunstream/services') as Array<{ id: string; availableTo: string[]; [key: string]: unknown }>;
    ...
    const associations = await sunsClient.call('/sunstream/associations') as Array<{ id: string; name: string; region: string; [key: string]: unknown }>;
    ...
    const catalogForTxn = await sunsClient.call('/sunstream/services') as Array<{ id: string; [key: string]: unknown }>;
    this.recordTransaction('sunstream', {
      messageId,
      type: 'onboarding',
      ...
      status: 'success',
      sourceData: {
        file: 'sunstream/service-catalog',
        recordId: catalogForTxn.length > 0 ? String(catalogForTxn[0].id) : 'svc-7001',
        recordType: 'ServiceCatalogItem',
      },
      summary: 'New association onboarding — first contact with SunStream, trust level UNVERIFIED',
    });
  ...
  // Scenario 11: Cross-Ecosystem — SunStream perspective
  // SunStream receives quality complaint data from Ascentek ecosystem and runs compliance validation
  private async scenario11_crossEcosystem(effectiveConfig: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown; messageId: string }> {
    ...
    const messageId = randomUUID();
    const tracer = new PipelineTracer({
      source: 'sunstream-ecosystem',
      target: 'sunstream-ecosystem',
      method: 'cross-ecosystem.receiveQualityComplaint',
    });

    // Step 1: Receive the cross-ecosystem request (simulated incoming from Ascentek)
    const incomingQualityComplaint = {
      ...
      originEcosystem: 'ascentek',
    };

    tracer.traceSync('Cross-Ecosystem Request Received', 'transport', 'a2a-jsonrpc', () => ({
      sender: 'ascentek-ecosystem',
      receiver: 'sunstream-ecosystem',
    ...
    tracer.traceSync('Quality Complaint Mapped to Compliance Schema', 'business', 'mcp', () => ({
      ...
      inputSchema: 'ascentek.quality-complaint',
      outputSchema: 'sunstream.loan-compliance-request',
      fieldsMapped: Object.keys(loanComplianceData).length,
      riskCategory: incomingQualityComplaint.severity === 'major' ? 'HIGH' : 'MEDIUM',
    }));

    // Step 3: Run SunStream compliance validation with the mapped data
    const complianceResult = this.sunstream.validateLoanCompliance(loanComplianceData);
    ...
    tracer.traceSync('Cross-Ecosystem Response Prepared', 'transport', 'a2a-jsonrpc', () => ({
      responseEcosystem: 'sunstream',
      targetEcosystem: 'ascentek',
    ...
    return {
      result: {
        ...
        perspective: 'sunstream-receiver',
        incomingRequest: incomingQualityComplaint,
        complianceValidation: complianceResult,
        note: 'Run scenario 11 on ascentek-app (port 4008) for the full cross-ecosystem orchestration view',
      },
      pipelineTrace,
      messageId,
    };
  ...
  private async scenario12_a2aFullSuite(effectiveConfig: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown; messageId: string }> {
    ...
    tracer.traceSync('A2A Agent Card Discovery', 'discovery', 'a2a-agent-card', () => ({
      url: 'http://localhost:4007/.well-known/agent-card.json',
      discoveredAgent: 'sunstream',
      skillId: skill.id,
  ...
  private async scenario13_agntcySecureExchange(effectiveConfig: ProtocolConfig): Promise<{ result: Record<string, unknown>; pipelineTrace: unknown; messageId: string }> {
    ...
    tracer.traceSync('AGNTCY OASF Lookup', 'discovery', 'agntcy-oasf', () => ({
      ...
      descriptorRef: 'oci://agntcy/sunstream:latest',
