 ...
 *
 * Prerequisites: ResearchHub (port 4001) must be running.
...
const RESEARCH_HUB_BASE = 'http://localhost:4001';
...
async function ensureResearchHubRunning(): Promise<void> {
  try {
    ...
    throw new Error(
      `ResearchHub is not running on port 4001. Start it with: npm run dev:research-hub\n${err}`,
    );
  }
}
