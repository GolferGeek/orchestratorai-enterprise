/**
 * FcsFinancialService security tests — 3 tests verifying that:
...
describe('FcsFinancialService security', () => {
  let service: FcsFinancialService;

  beforeEach(() => {
    service = new FcsFinancialService();
  ...
  it('submitLoanForCompliance: signing step uses SecurityService (not randomUUID)', async () => {
    ...
    expect(security['senderId']).toBe('fcs-financial');
  });
