...
/**
 * Unit tests for DataController (Ascentek app).
 *
 * These tests verify:
...
describe('DataController (ascentek)', () => {
  ...
  describe('successful responses', () => {
    it('getTransactions returns the DataFile from the loader', () => {
      const mockData = { filename: 'transactions', orgId: 'ascentek', records: [{ id: 't1' }], loadedAt: '2026-01-01' };
      __mockLoad.mockReturnValue(mockData);

      const result = controller.getTransactions('ascentek');
      expect(result).toBe(mockData);
      expect(__mockLoad).toHaveBeenCalledWith('ascentek', 'transactions');
    });

    it('getFile returns the DataFile from the loader', () => {
      const mockData = { filename: 'purchase-orders', orgId: 'oem-partner', records: [], loadedAt: '2026-01-01' };
      __mockLoad.mockReturnValue(mockData);

      const result = controller.getFile('oem-partner', 'purchase-orders');
      expect(result).toBe(mockData);
    });

    it('getRecord returns the record when found', () => {
      __mockLoad.mockReturnValue({ records: [], filename: 'purchase-orders', orgId: 'org', loadedAt: '' });
      const mockRecord = { id: 'po-001', poNumber: 'PO-2026-001' };
      __mockGetById.mockReturnValue(mockRecord);

      const result = controller.getRecord('org', 'purchase-orders', 'po-001');
