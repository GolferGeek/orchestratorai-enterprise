...
describe('AzureKeyVaultConfigProvider', () => {
  ...
  // Track env vars set by tests so we can clean them up
  ...
  beforeEach(() => {
    ...
    // Set process.env entries that match configValues for getSecret tests
    setProcessEnv('AZURE_KEYVAULT_URL', 'https://test-vault.vault.azure.net/');
    setProcessEnv('API_PORT', '6100');
    setProcessEnv('FEATURE_ENABLED', 'true');
    setProcessEnv('MAX_RETRIES', '3');
    setProcessEnv('MODEL_CONFIG', '{"provider":"openai","model":"gpt-4o"}');
    setProcessEnv('ANTHROPIC_API_KEY', 'sk-ant-from-env');
  ...
  afterEach(() => {
    // Clean up any process.env entries set during tests
  ...
  describe('getSecret', () => {
    ...
    it('fetches from Key Vault when not in env', async () => {
      ...
      deleteProcessEnv('ANTHROPIC_API_KEY');
    ...
    it('caches vault secrets after first fetch', async () => {
      ...
      // MY_SECRET is not in env (only in configValues)
    ...
    it('throws when secret not found in vault', async () => {
      ...
      // MISSING_SECRET is not set in env at all
    ...
    it('throws when vault secret has no value', async () => {
      ...
      // EMPTY_SECRET is not set in env at all
  ...
  describe('getSecretOptional', () => {
    it('returns default when secret not available', async () => {
      ...
      // MISSING is not set in env at all
  ...
  describe('vault name conversion', () => {
    it('converts underscores to hyphens and lowercases', async () => {
      ...
      // Remove from both configValues and process.env so it falls through to vault
      delete configValues.ANTHROPIC_API_KEY;
      deleteProcessEnv('ANTHROPIC_API_KEY');
