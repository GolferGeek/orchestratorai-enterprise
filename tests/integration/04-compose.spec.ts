/**
 * 04 — Compose API Integration Tests (port 6300)
 *
 * Real HTTP calls against the running Compose API.
 * Tests invoke, runners, RAG, auth proxy, and health endpoints.
 */
import { createTestClient, TestClient } from './helpers/http-client';
import { login, getExecutionContext, getUserContext } from './helpers/auth';
import { apiUrl } from './helpers/ports';
import { requireService } from './helpers/service-check';

const COMPOSE_BASE = apiUrl('compose');
let client: TestClient;
let orgSlug: string;

beforeAll(async () => {
  await requireService('compose');
  const token = await login();
  client = createTestClient(COMPOSE_BASE, token);
  const ctx = await getUserContext();
  orgSlug = ctx.organizations[0]?.slug ?? 'marketing';
});

// ─── Health ─────────────────────────────────────────────────────────────────

describe('Compose / Health', () => {
  it('GET /health returns healthy status', async () => {
    const res = await client.get<{ status: string }>('/health');
    expect(res.status).toBeDefined();
  });
});

// ─── Runners ────────────────────────────────────────────────────────────────

describe('Compose / Runners', () => {
  it('GET /runners returns available runners', async () => {
    const runners = await client.get<unknown>('/runners');
    expect(runners).toBeDefined();
  });
});

// ─── Invoke ─────────────────────────────────────────────────────────────────

describe('Compose / Invoke', () => {
  it('POST /invoke with JSON-RPC 2.0 format is accepted', async () => {
    const context = await getExecutionContext('test-agent', 'context');

    const res = await client.raw('/invoke', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        jsonrpc: '2.0',
        id: `e2e-${Date.now()}`,
        method: 'invoke',
        params: {
          context,
          data: { content: 'E2E test — hello', contentType: 'text' },
        },
      }),
    });

    // Accept 200 (success) or 500 (LLM key missing) — not 400/404
    expect([200, 500]).toContain(res.status);
  });
});

// ─── Auth Proxy ─────────────────────────────────────────────────────────────

describe('Compose / Auth Proxy', () => {
  it('GET /auth/me proxied to Auth API', async () => {
    const me = await client.get<{ id: string; email: string }>('/auth/me');
    expect(me.email).toBe('golfergeek@orchestratorai.io');
  });
});

// ─── RBAC ───────────────────────────────────────────────────────────────────

describe('Compose / RBAC', () => {
  it('GET /api/rbac/me/is-super-admin returns boolean', async () => {
    const result = await client.get<{ isSuperAdmin: boolean }>('/api/rbac/me/is-super-admin');
    expect(typeof result.isSuperAdmin).toBe('boolean');
  });
});

// ─── RAG Collections ────────────────────────────────────────────────────────

describe('Compose / RAG', () => {
  it('GET /api/rag/collections with org header returns data', async () => {
    const collections = await client.get<unknown>(
      '/api/rag/collections',
      { 'x-organization-slug': orgSlug },
    );
    expect(collections).toBeDefined();
  });
});

// ─── Feature Flags ──────────────────────────────────────────────────────────

describe('Compose / Feature Flags', () => {
  it('GET /feature-flags returns flags', async () => {
    const flags = await client.get<unknown>('/feature-flags');
    expect(flags).toBeDefined();
  });
});

